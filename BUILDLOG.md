...
- **Background Worker & Request Path Separation (Shared Requirement 3)**: Originally, alert evaluation was available as an on-demand trigger. Refactored architecture to decouple all slow/bulk post-metering calculations and threshold notification tasks completely off the HTTP request path using FastAPI `BackgroundTasks` via `BillingBackgroundWorker`. Persisted task lifecycles in `background_jobs`, added exponential backoff retries (up to 3 attempts), and implemented deterministic failure alerting emitting `[CRITICAL FAILURE ALERT]` and populating `job_failure_alerts` upon retry exhaustion. Added asynchronous bulk endpoint `POST /jobs/reconcile` returning `202 Accepted` immediately.
